...
$(LIB) : $(SRC_TO_COMPILE) \
	$(ZLIB) $(DRUNTIMELIB) win32.mak win32mscoff.mak
...
# build phobos32mscoff.lib
phobos32mscoff:
	$(MAKE) -f win32mscoff.mak "DMD=$(DMD)" "MAKE=$(MAKE)" MODEL=32mscoff "CC=\$(CC32)"\"" "AR=\$(AR)"\"" "VCDIR=$(VCDIR)" "SDKDIR=$(SDKDIR)"

# run unittests for 32-bit COFF version
unittest32mscoff:
	$(MAKE) -f win32mscoff.mak "DMD=$(DMD)" "MAKE=$(MAKE)" MODEL=32mscoff "CC=\$(CC32)"\"" "AR=\$(AR)"\"" "VCDIR=$(VCDIR)" "SDKDIR=$(SDKDIR)" unittest

######################################################

$(ZLIB): $(SRC_ZLIB)
	cd etc\c\zlib
	$(MAKE) -f win32mscoff.mak MODEL=$(MODEL) zlib$(MODEL).lib "CC=\$(CC)"\"" "LIB=\$(AR)"\"" "VCDIR=$(VCDIR)"
	cd ..\..\..
...
zip : win32.mak win32mscoff.mak posix.mak osmodel.mak $(STDDOC) $(SRC) \
	...
	del phobos.zip
	zip32 -u phobos win32.mak win32mscoff.mak posix.mak osmodel.mak $(STDDOC)
	zip32 -u phobos $(SRC)
...
clean:
	cd etc\c\zlib
	$(MAKE) -f win32mscoff.mak MODEL=$(MODEL) clean
	cd ..\..\..
	del $(DOCS)
